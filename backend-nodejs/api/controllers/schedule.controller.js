...
module.exports.getSlots = function (req, res, next) {
  ...
  if (!valid) {
    ...
    Schedule.findOne({
      $and: [{
        expertID: { $eq: req.params.expertID },
...
module.exports.expertOfferSlot = function (req, res, next) {
  ...
  if (!valid) {
    ...
    Schedule.findOneAndUpdate({
      $and: [{
        expertID: { $eq: req.decodedToken.user._id }, startDate: { $eq: start_date },
          ...
          Schedule.findOne({ $and: [{ expertID: { $eq: req.decodedToken.user._id }, startDate: { $eq: start_date }, endDate: { $eq: end_date } }] })
            .populate('slots.users', 'username').exec(function (err, schedule) {
              if (schedule) {
                ...
                Schedule.create({
                  expertID: req.decodedToken.user._id,
                  startDate: start_date, endDate: end_date, slots: req.body.slots
                }, function (err, createdSchedule) {
                  return res.status(201).json({
                    err: null,
...
module.exports.expertCancelSlot = function (req, res, next) {
  ...
  if (!valid) {
    ...
    Schedule.findOneAndUpdate({
      $and: [{
        expertID: { $eq: req.decodedToken.user._id }, startDate: { $eq: start_date }, endDate: { $eq: end_date },
      ...
      { $pull: { slots: req.body.slots } }).populate('slots.users', 'username').exec(function (err, updatedSchedule) {
        if (updatedSchedule) {
          ...
          NotificationController.createNotificationMuitiple(req.decodedToken.user._id, scheduleSlotUsers, " Cancelled The Slot", 0, function (done) {
  ...
  if (!valid) {
    ...
    Schedule.findOneAndUpdate({
      $and: [{
        ...
        slots: {
          $elemMatch: {
            day: { $eq: req.body.dayNo }, time: { $eq: req.body.slotNo },
        ...
        if (schedule) {
          ...
          if (!scheduleSession) {
            ...
            Session.create({ createdById: req.decodedToken.user._id, users: [req.body.userid] },
              function (err, session) {
                ...
                if (session) {
                  ...
                  Schedule.findOneAndUpdate({
                    _id: schedule._id, slots: {
                      $elemMatch: {
                        day: { $eq: req.body.dayNo },
            ...
            Session.findOneAndUpdate({
              _id: scheduleSession, users: { $ne: req.body.userid },
              ...
              { new: true }).populate('slots.users', 'username').exec(function (err, updatedSession) {
                ...
                if (updatedSession) {
                  NotificationController.createNotification(req.decodedToken.user._id, req.body.userid, "url/" + updatedSession._id, function (done) {
                    if (done) {
                      if (updatedSession.users.length >= maxNoUsers) {
                        ...
                        NotificationController.createNotificationMuitiple(req.decodedToken.user._id, scheduleSlotUsers,
                          " Rejected The Slot", 0, function (done) {
                            if (done) {
                              req.body.slots = { day: req.body.dayNo, time: req.body.slotNo }
...
module.exports.userReserveSlot = function (req, res, next) {
  ...
  if (!valid) {
    ...
    var end_date = ScheduleHelper.weekdayWithStartWeekday(Date.now(), 6, 6).format('D-MMMM-YY')
    Schedule.findOneAndUpdate({
      $and: [{ expertID: { $eq: req.body.expertID } }, { startDate: { $eq: start_date } },
      ...
      if (schedule) {
        NotificationController.createNotification(req.decodedToken.user._id, req.body.expertID, " Requested A Slot With", function (done) {
          if (done) {
            return res.status(201).json(
              {
                err: null,
...
module.exports.userUnReserveSlot = function (req, res, next) {
  ...
  if (!valid) {
    ...
    Schedule.findOneAndUpdate({
      ...
      if (schedule) {
        NotificationController.createNotification(req.decodedToken.user._id, req.body.expertID, " Cancelled A Slot Request With", function (done) {
          if (done) {
            return res.status(201).json(
              {
                err: null,
